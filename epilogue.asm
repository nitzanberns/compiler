...
L_code_ptr_bin_apply:
        lea rbx, [rsp + 8 * 4]           
        mov rax, qword [rbx]                     ;; rax = PARAM(1) = list
        mov rcx, 0                               ;; rcx = counter
.L_check_count_list:
        cmp rax, sob_nil 
        je .L_end_check_count_list               ;; arrived to the end of the list
        assert_pair(rax)                         ;; check if pair
        mov rax, SOB_PAIR_CDR(rax)               ;; prepare to check cdr
        inc rcx
        jmp .L_check_count_list
.L_end_check_count_list:                          
        lea rdx, [8 * (rcx + 2)]                ;; rcx = num of params
        sub rbx, rdx
        mov rdx, rsp
        mov rsp, rbx
        mov rdi, qword [rdx + 8 * 3]            ;; rdi = PARAM(0) = closure
        assert_closure(rdi)                     ;; check if closure
        mov r9, qword [rdx + 8 * 4]             ;; r9 = PARAM(1) = list
        mov rax, qword [rdx]
        mov qword [rbx], rax                    ;; 'push' ret addr
        add rbx, 8                              ;; move pointer
        mov rax, SOB_CLOSURE_ENV(rdi)           
        mov qword [rbx], rax                    ;; push env
        add rbx, 8                              ;; move pointer
        mov qword [rbx], rcx                    ;; 'push' n = rcx
        add rbx, 8                              ;; move pointer
 .L_extract_list:
        cmp rcx, 0              
        je .L_finish                            ;; finish pushing all list
        mov rax, SOB_PAIR_CAR(r9)
        mov qword [rbx], rax                    ;; 'push' car
        mov r9, SOB_PAIR_CDR(r9)                ;; prepare to push cadr
        add rbx, 8                              ;; move pointer
        dec rcx                                 ;; dec counter
        jmp .L_extract_list
 .L_finish:
